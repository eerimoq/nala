...
int hit();
# 3 "test_collect_tests.c" 2

static void all(void); static void all_before_fork() {} static struct nala_test_t nala_test_all = { .name_p = "all", .file_p = "test_collect_tests.c", .line = 4, .func = all, .before_fork_func = all_before_fork, .next_p = 
# 4 "test_collect_tests.c" 3 4
((void *)0) 
# 4 "test_collect_tests.c"
}; __attribute__ ((constructor)) static void nala_constructor_all(void) { nala_register_test(&nala_test_all); } static void all(void)
{
    foo_mock();
    bar_mock_once();
    fie_mock_ignore_in();

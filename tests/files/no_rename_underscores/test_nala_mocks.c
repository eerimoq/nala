...
struct nala_data_foo_t {
    ...
    int return_value;
    int errno_value;
    int (*implementation)(int __a);
    void (*callback)(int __a);
...
int __wrap_foo(int __a)
{
    ...
    case MODE_MOCK:
        ...
        if (nala_instance_p == NULL || nala_instance_p->mode == INSTANCE_MODE_NORMAL) {
            MOCK_ASSERT_IN_EQ(nala_data_p, foo, __a);
            nala_data_p->params_in.__a = __a;


            errno = nala_data_p->errno_value;

            if (nala_data_p->callback != NULL) {
                nala_data_p->callback(__a);
            }
    ...
    case MODE_IMPLEMENTATION:
        return_value =
        nala_mock_foo.data.implementation(__a);
        break;
...
void foo_mock(int __a, int return_value)
{
    ...
    nala_mock_foo.data.params.__a = __a;
    nala_mock_foo.data.params.ignore___a_in = false;
...
int foo_mock_once(int __a, int return_value)
{
    ...
    nala_instance_p->data.params.__a = __a;
    nala_instance_p->data.params.ignore___a_in = false;
...
void foo_mock_ignore_in(int return_value)
{
    CHECK_NO_INSTANCES(nala_mock_foo);
    nala_mock_foo.state.mode = MODE_MOCK;
    nala_mock_foo.data.params.ignore___a_in = true;
    nala_mock_foo.data.return_value = return_value;
    nala_mock_foo.data.errno_value = 0;
    nala_mock_foo.data.callback = NULL;
}
...
{
    nala_get_params_foo()->ignore___a_in = true;
}
...
{
    NALA_STATE_RESET(nala_mock_foo);
}
